...
class AutoGenerator {
    ...
    generateText() {
        ...
        tableNames.forEach(table => {
            ...
            const tableName = types_1.recase(this.options.caseModel, tableNameOrig, this.options.singularize);
            if (this.options.lang === 'ts') {
                ...
                needed.forEach(fkTable => {
                    ...
                    const [fkSchema, fkTableName] = types_1.qNameSplit(fkTable);
                    const filename = types_1.recase(this.options.caseFile, fkTableName, this.options.singularize);
                    str += 'import type { ';
                ...
                if (primaryKeys.length) {
                    str += `export type #TABLE#Pk = ${primaryKeys.map((k) => `"${types_1.recase(this.options.caseProp, k)}"`).join(' | ')};\n`;
    ...
    addSchemaForRelations(table) {
        if (!this.relations.some(rel => rel.childTable === table)) {
            ...
            if (first) {
                const [schemaName, tableName] = types_1.qNameSplit(first.childTable);
                if (schemaName) {
                    table = types_1.qNameJoin(schemaName, table);
                }
            }
        }
        return table;
            ...
            if (!rel.isM2M) {
                if (rel.childTable === table) {
                    ...
                    if (rel.isOne) {
                        ...
                        const sing = lodash_1.default.upperFirst(sequelize_1.Utils.singularize(rel.childProp));
                        const lur = sequelize_1.Utils.pluralize(rel.childProp);
                ...
                if (rel.parentTable === table) {
                    ...
                    const otherModelSingular = lodash_1.default.upperFirst(sequelize_1.Utils.singularize(isParent ? rel.childProp : rel.parentProp));
                    const lotherModelPlural = sequelize_1.Utils.pluralize(isParent ? rel.childProp : rel.parentProp);
    ...
    getTypeScriptAllowNull(table, field) {
        const fieldObj = this.tables[table][field];
